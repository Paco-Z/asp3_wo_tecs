 ...
 */
#define OVRTIMER_TIMER_ID		0
#define OVRTIMER_TIMER_CH		0
#define OVRTIMER_TIMER_IRQNO	TTC_ID0_CH0_INTNO
...
Inline uint32_t
_prctim_to_cnt(PRCTIM prctim) {
	if (prctim < OVRTIMER_TIMER_TICK_NS) {
		return 1;
	}
	else {
		return (prctim/OVRTIMER_TIMER_TICK_NS) + 1;
	}
}

Inline PRCTIM
_cnt_to_prctim(uint32_t cnt) {
	return cnt * OVRTIMER_TIMER_TICK_NS;
}
 ...
 */
Inline void
_target_ovrtimer_start(PRCTIM ovrtim)
{
	sil_wrw_mem(TTC_INTERVAL_COUNTER(OVRTIMER_TIMER_ID, OVRTIMER_TIMER_CH),_prctim_to_cnt(ovrtim));
	sil_wrw_mem(TTC_COUNTER_CONTROL(OVRTIMER_TIMER_ID, OVRTIMER_TIMER_CH),
				TTC_COUNTER_DEC|TTC_COUNTER_RST|TTC_COUNTER_INT);
 ...
 */
Inline PRCTIM
_target_ovrtimer_stop(void)
{
	uint32_t cnt;

	sil_wrw_mem(TTC_COUNTER_CONTROL(OVRTIMER_TIMER_ID, OVRTIMER_TIMER_CH),TTC_COUNTER_DIS);
	cnt = sil_rew_mem(TTC_COUNTER_VALUE(OVRTIMER_TIMER_ID, OVRTIMER_TIMER_CH));

	return(_cnt_to_prctim(cnt));
}
 ...
 */
Inline PRCTIM
_target_ovrtimer_get_current(void)
{
	uint32_t cnt;
...
void
_target_ovrtimer_initialize(intptr_t exinf)
{
 ...
 */
void
_target_ovrtimer_terminate(intptr_t exinf)
{
	sil_wrw_mem(TTC_COUNTER_CONTROL(OVRTIMER_TIMER_ID, OVRTIMER_TIMER_CH),TTC_COUNTER_DIS);
}
...
void cyclic_handler(intptr_t exinf)
{
	...
	SVC_PERROR(rot_rdq(MID_PRIORITY));
	SVC_PERROR(rot_rdq(LOW_PRIORITY));
	syslog(LOG_NOTICE, "Cyclie %d!", cnt++);
}
...
//	SVC_PERROR(act_tsk(TASK3));
	...
	do {
		...
		case 'Z':
			...
			break;
		case '1':
			_target_ovrtimer_initialize(0);
			_target_ovrtimer_start(6000000);
			syslog(LOG_INFO, "cnt %d", _target_ovrtimer_get_current());
			tskno = 1;
			tskid = TASK1;
			break;
		case '2':
			syslog(LOG_INFO, "cnt %d", _target_ovrtimer_get_current());
//			_target_ovrtimer_initialize(0);
//			target_ovrtimer_start(0);
			tskno = 2;
			tskid = TASK2;
			break;
		case '3':
			syslog(LOG_INFO, "flag 0x%x", sil_rew_mem(TTC_INTERRUPT_REGSISTER(OVRTIMER_TIMER_ID, OVRTIMER_TIMER_CH)));
			_target_ovrtimer_stop();
			syslog(LOG_INFO, "cnt %d", _target_ovrtimer_get_current());
			tskno = 3;
			tskid = TASK3;
			break;
